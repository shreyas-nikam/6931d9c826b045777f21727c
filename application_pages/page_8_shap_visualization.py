...
def main():
    ...
    if not case_options:
        st.warning("No rejected cases with SHAP explanations found. Please generate SHAP explanations first.")
        return
    ...
    if f"shap_explainer_{selected_case_id}" not in st.session_state or f"shap_values_{selected_case_id}" not in st.session_state:
        st.warning(f"SHAP explanation for Applicant ID {selected_case_id} not found. Please generate it on the 'SHAP Explanation' page first.")
        return
    ...
    application_data_features = st.session_state["loan_data"].loc[selected_case_id][st.session_state["model_features"]]
    ...
    shap.dependence_plot(
        ...
        shap_values=st.session_state[f"shap_values_{selected_case_id}"], # Pass shap_values array directly if available
